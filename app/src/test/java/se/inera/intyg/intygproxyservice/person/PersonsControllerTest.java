...
class PersonsControllerTest {
  ...
  @Mock private PersonsService personsService;

  @InjectMocks private PersonsController personsController;

  @Test
  void shallReturnPersonResponseWhenCallingFindPerson() {
    final var expectedPersonsResponse = PersonsResponse.builder().build();

    doReturn(expectedPersonsResponse).when(personsService).findPersons(any(PersonsRequest.class));

    final var actualPersonResponse =
        personsController.findPersons(PersonsRequest.builder().build());
