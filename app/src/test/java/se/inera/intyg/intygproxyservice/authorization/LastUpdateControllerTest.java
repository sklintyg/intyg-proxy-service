...
class LastUpdateControllerTest {
  ...
  private static final GetLastUpdateResponse EXPECTED =
      GetLastUpdateResponse.builder().lastUpdate(LocalDateTime.now()).build();

  @Mock GetLastUpdateService getLastUpdateService;

  @InjectMocks LastUpdateController lastUpdateController;

  @Test
  void shouldReturnValueFromService() {
    when(getLastUpdateService.get()).thenReturn(EXPECTED);
