 ...
 *
 * <p>NOTE: Copied from schemas and updated to java17
 */
public final class LocalDateAdapter {
  ...
  private static final DateTimeFormatter ISO_DATETIME_FORMATTER =
      new DateTimeFormatterBuilder()
          .appendPattern(ISO_DATETIME_PATTERN)
          .appendFraction(ChronoField.NANO_OF_SECOND, 0, 9, true)
          .toFormatter();
  private static final String XSD_DATE_TIMEZONE_REGEXP = "\\d{4}-\\d{2}-\\d{2}([+-].*|Z)";
  private static final String XSD_DATETIME_TIMEZONE_REGEXP =
  ...
  public static LocalDate parseDate(String dateString) {
    ...
    if (dateString.matches(XSD_DATE_TIMEZONE_REGEXP)
        || dateString.matches(XSD_DATETIME_TIMEZONE_REGEXP)) {
  ...
  public static LocalDateTime parseDateTime(String dateTimeString) {
    // Workaround for the fact that DatatypeConverter doesn't allow setting the default TimeZone,
    // (which means that the system default TimeZone will be used, which might be wrong if hosted in
    // a different TimeZone), and that LocalDateTime doesn't handle datetimes with explicit
    // TimeZone.
    // Hence if the date contains an explicit TimeZone, use DatatypeConverter to do the parsing,
    ...
    if (dateTimeString.matches(XSD_DATETIME_TIMEZONE_REGEXP)
      ...
      return LocalDateTime.from(
          DatatypeConverter.parseDateTime(dateTimeString).toInstant().atZone(TIMEZONE));
